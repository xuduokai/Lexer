...
public class MinimizeDFA {
    ...
    public MinimizeDFA(DfaConstructor theConstructor, ArrayList<Character> charSet) {
        ...
        while (it.hasNext()) {
            ...
            for (Character c : charSet) {
                if (dfaTransTable[from][c] != STATE_FAILURE) {
    				 ...
    				 */
                    minDfa[fromGroup.groupNum()][charSet.indexOf(c)] = toGroup.groupNum();
                }
            }
        }
    }
    ...
    private void initMiniDfaTransTable() {
        minDfa = new int[groupManager.size()][charSet.size()];
        for (int i = 0; i < groupManager.size(); i++)
